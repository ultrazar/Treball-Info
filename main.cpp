...
void MedioImposicionMasComun (const v_denuncia&v1, const v_imposicio&v3){
    ...
    cout << "La denuncia más común tiene como código " << impmascomun << " y ha tenido " << mascomun << " incidencias, ";
    cout << "hecho que representa el " << (float(mascomun)/v1.size())*100 << "% de las denuncias." << endl;
...
void ValidacionDatos(const v_denuncia&v1, const v_multa&v2, const v_imposicio&v3, const t_zonas&z) {
    ...
    for (int i = 0; i < v1.size() && !salir; i++) {
        ...
        if ((v1[i].mes > 12 || v1[i].mes < 0) && v1[i].mes != -1) {
            error = true;
            error_causa = "Mes incorrecto";
        }
        else if ((v1[i].dia > 31 || v1[i].dia < 0) && v1[i].dia != -1) {
            error = true;
        ...
        // Validamos Zonas
        else if (v1[i].codi_districte > 10 && v1[i].codi_districte != -1) {
            error = true;
            error_causa = "Distrito inválido";
        }
        else if (v1[i].codi_barri > 74 && v1[i].codi_barri != -1) {
             error = true;
             error_causa = "Barrio inválido";
        }

        //Validamos si existe la multa
        else if (!error && v1[i].infraccio_codi != -1) {
            trobat = false;
            ...
            if(!trobat) {
               ...
               error = true;
               error_causa = "Importe inválido";
               errorimporte = true;
            }
        }

        //Validamos si existe el subtipo
        else if (!error && v1[i].subtipus_exp != "" && v1[i].subtipus_exp != "-1") {
            trobat = false;
            for(int k = 0; k < v3.size() && !trobat; k++) {
                if(v3[k].concepte == v1[i].subtipus_exp) {
                        trobat = true;
                }
            }
            if(!trobat) {
        ...
        else if (!error && v1[i].mitja_impos != "" && v1[i].mitja_impos != "-1") {
            trobat = false;
            for(int k = 0; k < v3.size() && !trobat; k++) {
                if(v3[k].concepte == v1[i].mitja_impos) {
                    trobat = true;
                }
            }
            if(!trobat) {
